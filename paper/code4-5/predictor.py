...
def per_sample_metrics(predictor: AeroGtoPredictor,
    ...
    active_mask = build_active_mask(gt_norm, fields, mask_cfg)  # [T, N, C]，与 state 同形
    if active_mask is None:
        active_mask = np.ones(gt_real.shape[:2], dtype=bool)
    else:
        if torch.is_tensor(active_mask):
            active_mask = active_mask.cpu().numpy()
        # 跨通道取并集：任一受关注通道处于活跃区即视为活跃节点，得到 [T, N]
        active_mask = active_mask.astype(bool).any(axis=-1).reshape(gt_real.shape[:2])
