...
```bash
conda activate GTO
cd /home/ubuntu/PhysGTO/Finalexp
...
python paper/code5/error_spatial.py \
  ...
  --config config/keyhole/time_stride2/GTO_attnres_3_keyhole_stronger_time_stride2.json \
  --mode_label keyhole \
  --time_step 10 --slice_axis y
...
# 4.5.3 — 失败案例剖析（默认 top-5，按 overall rL2 排序）
