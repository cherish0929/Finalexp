...
# 引入项目模块
from src.dataset import CutAeroGtoDataset
...
class AeroGtoPredictor:
    ...
    def predict_rollout(self, sample_idx, interface_field="alpha.air"):
        ...
        pos_min, pos_max = torch.from_numpy(meta["pos_min"]).to(self.device), torch.from_numpy(meta["pos_max"]).to(self.device)

        raw_coords:torch.Tensor = node_pos * (pos_max - pos_min + 1e-8) + pos_min
        raw_coords = raw_coords.squeeze().cpu().numpy()
...
if __name__ == "__main__":
    MODE = "test"
    # === 配置区域 ===
    # CONFIG_PATH = "config/easypool/cut_GTO_attnres_3_easypool.json"
